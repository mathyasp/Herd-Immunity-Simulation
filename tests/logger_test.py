...
if __name__ == '__main__':
    file_name = '../logs/test_logger_file.txt'
    virus_name = 'Sniffles'
    repro_num = 0.5
    mortality_rate = 0.12
    pop_size = 1000
    ...
    assert new_logger.file_name == '../logs/test_logger_file.txt'


    # Test log file was created + test write_metadata method 
    new_logger.write_metadata(pop_size, vacc_percentage, virus_name, mortality_rate, repro_num, date)

    file = open(file_name, 'r')
    write_test = file.read()

    assert write_test == 'Population Size: 1000\nVaccination Percentage: 0.1\nVirus Name: Sniffles\nMortality Rate: 0.12\nBasic Reproduction Number: 0.5\nDate of Simulation: 10/10/2023\n'
